...
def load_prices(ticker: str, csv_path: str = None, period: str = "3y") -> pd.DataFrame:
    """Read a per-ticker prices.csv (e.g. AAPL/prices.csv) if it exists,
...
if __name__ == "__main__":
    # Ticker folders live at the repo root (e.g. ./AAPL/prices.csv). Run this
    # script from the repo root so the relative paths resolve.
    TICKERS = ["AAPL", "AMZN", "MSFT", "NVDA", "PEP"]

    # SPY is the market benchmark for the "vs SPY" metric. There is no SPY/
    # folder in the repo, so try to download it; skip the benchmark if offline.
    try:
        spy = load_prices("SPY", csv_path="SPY/prices.csv")["Close"]
    except Exception as e:
        print(f"[note] SPY benchmark unavailable ({e}); cap_vs_spy will be NaN.\n")
        spy = None
    ...
    for ticker in TICKERS:
        price = load_prices(ticker, csv_path=f"{ticker}/prices.csv")
